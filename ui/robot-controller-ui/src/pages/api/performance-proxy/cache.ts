...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    switch (profile) {
      case 'tailscale':
        gatewayUrl = `http://${process.env.NEXT_PUBLIC_ROBOT_HOST_TAILSCALE}:7070/api/performance/cache`;
        break;
      case 'lan':
        gatewayUrl = `http://${process.env.NEXT_PUBLIC_ROBOT_HOST_LAN}:7070/api/performance/cache`;
        break;
      case 'local':
        gatewayUrl = `http://${process.env.NEXT_PUBLIC_ROBOT_HOST_LOCAL}:7070/api/performance/cache`;
        break;
      default:
        gatewayUrl = `http://omega1.local:7070/api/performance/cache`;
    }
